...
class NeRF(torch.nn.Module):
    def __init__(self,opt):
        ...
        self.density_activ = getattr(torch_F,opt.arch.density_activ) # relu_,abs_,sigmoid_,exp_....
...
class NeRFTiny(torch.nn.Module):
    ...
    def define_network(self, opt):
        ...
        self.point_3d_encoding = tcnn.Encoding(3, NeRFTiny.rgb_encoding_config, dtype=torch.float)

        # 63 x 128 in 4 layers
        self.point_3d_density_and_feature_1 = tcnn.Network(input_3D_dim, NeRFTiny.layer_width, NeRFTiny.network_config)

        # skip, (128 + 63) x 128 in 4 layers
        self.point_3d_density_and_feature_2 = tcnn.Network(NeRFTiny.layer_width + input_3D_dim,
                                                           NeRFTiny.layer_width + 1,
                                                           NeRFTiny.network_config)

        # view dependency
        # view dependent encoding, encode ray direction
        self.view_dependent_encoding = tcnn.Encoding(3, NeRFTiny.view_encoding_config, dtype=torch.float)

        # view dependent MLP
        self.rgb_feature_direction_rgb = tcnn.Network(NeRFTiny.layer_width + input_view_dim,
                                                      3,
                                                      NeRFTiny.network_config)

    def forward(self, opt, points_3D, ray_unit=None, mode=None):
        batch_size = points_3D.size()[0]
        samples_size = points_3D.size()[1]
        ...
        rgb = rgb.reshape(batch_size, samples_size, -1, 3)
        depth = depth.reshape(batch_size, samples_size, -1)
        return rgb, depth
